    ...
    (acceptedFiles: FileWithPath[], rejectedFiles: FileRejection[]) => {
      ...
      if (rejectedFiles.length > 0) {
        rejectedFiles.forEach(({ errors }) => {
          ...
          errors[0]?.message && toast({ title: "Error", description: errors[0]?.message });
        });
      }
    },
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="outline" disabled={disabled}>
          Subir Imagenes
          <span className="sr-only">Subir Imagenes</span>
        </Button>
      </DialogTrigger>
          ...
          {isUploading ? (
            ...
            <div className="grid place-items-center gap-2 text-muted-foreground sm:px-5">
              <UploadIcon
                ...
                aria-hidden="true"
              />
              <p className="text-base font-medium">Arrastra el Archivo aca</p>
            </div>
          ) : (
            <div className="grid place-items-center gap-1 sm:px-5">
              ...
              <p className="mt-2 text-base font-medium text-muted-foreground">
                Arrastra y sueelta el archivo o haz click para seleccionar
              </p>
              <p className="text-sm text-slate-500">
                El tamaño maximo es {formatBytes(maxSize)}
              </p>
            </div>
          )}
        </div>
        <p className="text-center text-sm font-medium text-muted-foreground">
          Puedes subir hasta {maxFiles} {maxFiles === 1 ? "archivo" : "archivos"}
        </p>
        {files?.length ? (
